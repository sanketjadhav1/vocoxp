...
$check_error_res = check_error($mysqli, $mysqli1);
if ($check_error_res == 1) {
	            	...
	            	if ($verification_data['data']['code'] == 1000) 
	            	{
	            		...
	            		$polling_station=$verification_data['data']['voter_data']['polling_station'];
	            		 ...
	            		 $fetch_wallet = "SELECT `current_wallet_bal` FROM `agency_header_all` WHERE `agency_id`='$agency_id'";
							...
							$insert_pan_payment = "INSERT INTO `direct_verification_details_all` (`direct_id`, `application_id`, `agency_id`, `verification_id`, `initiated_on`, `completed_on`, `activity_status`, `linked_table`, `deducted_base_amount`, `sgst_amount`, `cgst_amount`,  `source_from`) VALUES ('$direct_id', '$application_id', '$agency_id', '$verification_id', '$system_date_time', '$system_date_time', '$status', 'direct_voter_details_all', '$base_amount', '$sgst_amt', '$cgst_amt',  '$source_from') ";
							$res_pan = mysqli_query($mysqli, $insert_pan_payment);

							// Insert into `direct_pan_details_all`

							$insert_pan_detail = "INSERT INTO `direct_voter_details_all` (`id`, `direct_id`, `application_id`, `agency_id`, `admin_id`, `initiated_on`, `completed_on`, `activity_status`, `voter_number`, `name`, `gender`, `father_name`, `address`, `polling_details`, `user_photo`) VALUES (NULL, '$direct_id', '$application_id', '$agency_id', '$admin_id', '$system_date_time', '$system_date_time', '1', '$user_voter_number', '$name',  '$gender', '$father_name', '$part_name', '$polling_station', '$user_img') ";
	        ...
	        {
	        	 $responce = ["error_code" => 199, "message" => "please enter correct verification_for mode"];
	                echo json_encode($responce);
	                return;
	        }
...
function check_error($mysqli, $mysqli1)
{
