...
$failed_transactions = [];
    $fetch_data_debit = "SELECT `user_id`, `base_amount`, `purchase_type`, `cgst_amount`, `sgst_amount`, `transaction_on`, `verification_id`, 
                            (`base_amount` + `sgst_amount` + `cgst_amount`) AS amount , `requested_from`
...
foreach ($all_record as $wallet) {
    ...
    if (isset($wallet['user_id'])) {
        ...
        $stmt_veri_name = $mysqli1->prepare("SELECT `name`, `abbreviations` FROM `verification_header_all` WHERE `verification_id` = ?");
        ...
        if ($user_pre[0] == "MEM") {
            ...
            $res_name = $stmt_member->get_result();
            $arr_name = $res_name->fetch_assoc();
        } else if ($user_pre[0] == "VIS") {  // get visitor details
            $stmt_member = $mysqli->prepare("SELECT `visitor_name` AS `name` FROM `visitor_temp_activity_detail_all` WHERE `visitor_id` = ? AND `agency_id` = ?");
            $stmt_member->bind_param("ss", $user_id, $agency_id);
            $stmt_member->execute();
            $res_name = $stmt_member->get_result();
            $arr_name = $res_name->fetch_assoc();
            ...
            if (empty($arr_name)) {
                /* if temp details are empty then get data from header */
                $stmt_member = $mysqli->prepare("SELECT `visitor_name` AS `name` FROM `visitor_header_all` WHERE `visitor_id` = ? AND `agency_id` = ?");
                $stmt_member->bind_param("ss", $user_id, $agency_id);
                $stmt_member->execute();
                $res_name = $stmt_member->get_result();
                $arr_name = $res_name->fetch_assoc();
...
$tax_amt=$wallet['cgst_amount']+$wallet['sgst_amount'];
    $transaction_array[] = [
        ...
        $row1['abbreviation'] = $arr_veri_name['abbreviations'] ?? "",
        $row1['payment_via'] = "wallet  ",
