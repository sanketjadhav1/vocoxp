...
// die;
            //  $sql_1 = "id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, ".$data[0][1].' int, '.$data[0][2].' varchar(25), '.$data[0][3].' varchar(100), '.$data[0][4].' varchar(100), '.$data[0][5].' varchar(100), '.$data[0][6].' int, '.$data[0][7].' varchar(50)';
            //  $sql_11= "create table t1($sql_1)";
            // $result = $db->query($sql_11);
            // echo $data[1][1];
            for($i=0,$j=1;$i<=50;$i++,$j++){
                if($data[$i][1]!=""){
                    echo $data[$i][1];
                    // $a=$data[$i][1];
                  echo  $sql_2="insert into t1(`serial_number`,`voter_id`,`name`,`fathers_husbands_name`,`house_number`,`age`,`gender`) values('".$data[$i+1][1]."','".$data[$i+1][2]."','".$data[$i+1][3]."','".$data[$i+1][4]."','".$data[$i+1][5]."','".$data[$i+1][6]."','".$data[$i+1][7]."')";
                    $result_03 = mysqli_query($db, $sql_2);
