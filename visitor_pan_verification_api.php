...
if ($check_error_res == 1) {
    ...
    try {
        ...
        //get pan card data for verification
        $verification_data = json_decode(verify_pan($user_pan_number), true);
        $original_pan_details = $verification_data['data'];

        if ($original_pan_details['status'] == "INVALID") {
            $responce = ["error_code" => 199, "message" => "Pan number is invalid. Please provide the valid pan number"];
            echo json_encode($responce);
            return;
        }

        $ver_pan_name_arr = explode(" ", strtoupper($original_pan_details['full_name']));
        $temp_pan_name_arr = explode(" ", strtoupper(trim($visitor_temp_detail_arr['visitor_name'], " ")));

        $name_arr_cnt = 0;
        $name_match_cnt = 0;
        if (sizeof($ver_pan_name_arr) > sizeof($temp_pan_name_arr)) {
            $name_arr_cnt = sizeof($ver_pan_name_arr);
        } else {
            $name_arr_cnt = sizeof($temp_pan_name_arr);
        }
        foreach ($ver_pan_name_arr as $val) {
            if (in_array($val, $temp_pan_name_arr)) {
                $name_match_cnt++;
            }
        }
        ...
        if ($is_name_match == 'Not Match') {
            $ambiguity = 'name@' . $original_pan_details['full_name'] . '!' . $visitor_temp_detail_arr['visitor_name'];
            $ambiguity_details = $ambiguity;
            $match_all = 0;
            $color = 'red';
        }
        /* dob & father_name details are not getting from pan card api
        if ($original_pan_details['dob'] != $visitor_temp_detail_arr['dob']) {
            $ambiguity = 'dob@' . $original_pan_details['dob'] . '!' . $visitor_temp_detail_arr['dob'];
            $ambiguity_details .= ($ambiguity_details != '') ? ',' . $ambiguity : $ambiguity;
            $match_all = 0;
        }
        if ($original_pan_details['father_name'] != $visitor_temp_detail_arr['father_name']) {
            $ambiguity = 'father_name@' . $original_pan_details['father_name'] . '!' . $visitor_temp_detail_arr['father_name'];
            $ambiguity_details .= ($ambiguity_details != '') ? ',' . $ambiguity : $ambiguity;
            $match_all = 0;
        } */
...
<div id="firstDiv">
    <table style="width:100%; border-collapse: collapse;">
        ...
        <tbody>
            <tr>
                <td width="70%">
                    <h3 id="header"> PAN CARD VERIFICATION REPORT</h3>
                </td>
...
<table style=" width:100%; border-collapse: collapse;" id="table">
<tr>
<td scope="col" style="width:50%"><span style="font-weight: bold;">Pan Card Number:</span> ' . $visitor_temp_detail_arr['pan_number'] . '</td>

<td scope="col" style="width:50%"><span style="font-weight: bold;"> Name:</span> ' . strtoupper($original_pan_details['full_name']) . '<br><span style="color:' . $color . ';">' . $is_name_match . '</span></td>
</tr>
<tr>
</table>
...
<p style="margin-top:10px;"><span style="color:red;">*</span>Please note that the contents shown above is provided in good faith, we do not warrant that the information will be kept up to date, be true, accurate and not misleading.</p>
    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
        ...
        function savePanCardPDF($output_pdf, $visitor_id, $agency_id, $emp_id)
        {
            ...
            $new_directory_path = "$agency_id/visitor/$emp_id/pan_card_report/";
            ...
            $base_url = "https://mounarchtech.com/central_wp/verification_data/voco_xp/$agency_id/visitor/$emp_id/pan_card_report";
        ...
        // Save PDF to a file
        $pdf_path = savePanCardPDF($output_pdf, $visitor_id, $agency_id, $emp_id);
        ...
        $insert_pan_query = "INSERT INTO `visitor_pan_details_all` (`visitor_id`,`agency_id`,`initiated_on`,`completed_on`,`activity_status`,`pan_number`,`name`,`father_name`,`dob`,`user_photo`,`front_photo`,`generated_by`,`is_athenticate`,`pan_ambiguity`,`report_url`) VALUES ('$visitor_id','$agency_id','$system_datetime','$system_datetime','$activity_status','{$visitor_temp_detail_arr['pan_number']}', '{$visitor_temp_detail_arr['visitor_name']}','{$visitor_temp_detail_arr['father_name']}','{$visitor_temp_detail_arr['dob']}','{$visitor_temp_detail_arr['user_photo']}','{$visitor_temp_detail_arr['front_photo']}','{$visitor_temp_detail_arr['mode']}','1','$ambiguity_details','$pdf_path')";

        $insert_pan_detail =  $mysqli->query($insert_pan_query);
        if (!$insert_pan_detail) {
            throw new Exception("Failed to Insert into visitor_pan_details_all");
        }
...
/* get pan card details on pan no. */
function verify_pan($pancard_no)
{
    $curl = curl_init();
...
/* get access token to get pan card details */
function get_token()
{
    $curl = curl_init();
    curl_setopt_array($curl, [
