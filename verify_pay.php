...
// Your Razorpay API credentials
 ...
 // $cgst_amount = $_GET['cgst_amount'];
 // $sgst_amount = $_GET['sgst_amount'];
 // $mi_amount = $_GET['mi_amount'];
 // $mi_sgst_amount = $_GET['mi_sgst_amount'];
 // $mi_cgst_amount = $_GET['mi_cgst_amount'];
 // $agency_amount = $_GET['agency_amount'];
 // $agency_sgst_amount = $_GET['agency_sgst_amount'];
 // $agency_cgst_amount = $_GET['agency_cgst_amount'];
...
if ($status == 'paid') {
     ...
     $pay_check = "SELECT * FROM end_user_payment_transaction_all WHERE end_user_id = '$enduser_id' AND gateway_transaction_id = '$payment_id'";
        ...
        $select_array = mysqli_fetch_assoc($result_paychk);
            $fetch_amount="SELECT `generated_for`,`client_addon_amt`,`verifications`,`amount`,`child_verifications`,`child_amount`,`child_client_addon_amt` FROM `bulk_weblink_request_all` WHERE `agency_id`='$agency_id' AND `bulk_id`='$weblink_id'";
         ...
         $html = '<html>
                ...
                // Save PDF to a file
                 $path = savePDF1($output_pdf, $enduser_id, $agency_id);
                     ...
                     $update_wallet = "UPDATE `bulk_end_user_transaction_all` SET `payment_invoice`='$path' WHERE `agency_id`='$agency_id' AND `bulk_id`='$weblink_id' AND `end_user_id`='$enduser_id'";
