...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ($res_agency && mysqli_num_rows($res_agency) > 0) {
        ...
        $weblink_details = "SELECT `verifications`,`request_no`,`generated_for` FROM `bulk_weblink_request_all` WHERE `agency_id`='$agency_id' AND `bulk_id` = '$bulk_id' AND (`status` = '1' OR `status` = '2')";
        $weblink_result = $mysqli->query($weblink_details);
        $weblink_array = mysqli_fetch_assoc($weblink_result);
        if (is_array($weblink_array) && isset($weblink_array['verifications']) && isset($weblink_array['request_no'])) {
...
<head>
    ...
    margin-left: -2vw !important;
        ...
        .selectcss {
            ...
            /* Optional: For rounded corners */
            padding: 5px;
            /* Optional: To add padding inside the select box */
            background-color: #fff;
            /* Ensure background color is white */
            box-sizing: border-box;
            /* Ensure padding and border are included in the element's total width and height */
        ...
        /* To change the border color when the select box is focused */
        .selectcss:focus {
            border-color: red;
...
<body>
    ...
    // Check if the agency logo exists
            ...
            <div class="wizard-form">
                ...
                <div class="wizard-body" style="display:none" id="wizard-bodyDiv">
                    <div class="containers">
                        <span for="verification-for"><b>Weblink For: </b><span id="verification-for"></span></span><br>
                    </div>
                    ...
                    <div id="confighide" style="display: block;">
                        <h4>Settings</h4>
                ...
                <section id="pricing-section">
                    <div class="inner">
                        ...
                        <div id="table_1" class="table-container" style="display: none;">
                            ...
                            <p class="text-danger" style="display:none;" id="low_balance">Please Add the balance to your vallet</p>
                            <div class="form-row">
                                <div class="form-holder">
                                    <label class="form-row-inner">
                                        <table class="table table-bordered">
                        ...
                        <div id="table_2" class="table-container" >
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#next_slide a[href$="#next"]', function(e) {
                ...
                if (!req_no) {
                    hasError = true;
                    $('#request-no').addClass('error'); // Optionally add a class for styling
                } else {
                ...
                if (!upload_excel) {
                    hasError = true;
                    $('#upload-excel').addClass('error'); // Optionally add a class for styling
                } else {
        ...
        function test1() {
            ...
            var bulkId = bulkIdElement.value; // Retrieve the bulk ID
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.error) {
                        ...
                        if (response.generated_for == '1') {
                            $('#verification-for').text('School-Students');
                        } else if (response.generated_for == '2') {
                            $('#verification-for').text('School-Parents');
                        } else if (response.generated_for == '3') {
                            $('#verification-for').text('School-Parents & Students');
                        } else if (response.generated_for == '4') {
                            $('#verification-for').text('Institute-Students Above 16yrs');
                        } else if (response.generated_for == '5') {
                            $('#verification-for').text('Employee\'s');
                        } else if (response.generated_for == '6') {
                            $('#verification-for').text('On-boarding');
                        } else if (response.generated_for == '7') {
                            $('#verification-for').text('Hostel-Services(Students)');
                        } else if (response.generated_for == '8') {
                            $('#verification-for').text('Hostel-Services(Students & Parents)');
                        } else if (response.generated_for == '9') {
                            $('#verification-for').text('Campus Counseling');
                        } else if (response.generated_for == '10') {
                            $('#verification-for').text('Custom');
                        } else {
                        ...
                        `;
                            ...
                            // let validRows =$('#validRows').val();
                            validRows = parseInt($('#validRows').val(), 10) || 0;
                            ...
                            // Function to update the all_total row
                            function updateTotalRow() {
                                let totalAmount = 0;

                                // Loop through each row in tbody_1
                                ...
                                if (!allTotalRow) {
                                    ...
                                    allTotalRow.innerHTML = `
                                           <td>all_total</td>
                                                    <td colspan="3">${totalAmount.toFixed(2)}</td> <!-- colspan to merge cells -->
                                                         `;
                                                         
                               
                                    tbody_1.appendChild(allTotalRow);
                                } else {
                                    allTotalRow.cells[1].innerText = totalAmount.toFixed(2);
                                }
                                // alert(totalAmount);
                                // alert(current_wallet_bal);
                                if(current_wallet_bal < totalAmount ){
                        ...
                        if (response.child_verifications && response.child_verifications.trim() !== "") {
                            var students = document.getElementById("students");
                            students.style.display = "block";
                            var child_verifications = response.child_verifications.split(',');
                            let newRowchdis = document.createElement('tr');
                            newRowchdis.innerHTML = `
    <td colspan="4" ><input type="text" style="width: 100%;" value="Students Verifications Details" disabled></td>
`;
                            tbody1.appendChild(newRowchdis);
                            tbody2.appendChild(newRowchdis);
                            child_verifications.forEach(function(element) {
                                let chverificationType = '';
                                let reserveRate = '';
                                let totalAmount = '';
                                let taxAmount = '';
                                let grandAmount = '';
                                ...
                                divElement.innerHTML = '<button type="button" class="otp-button">' + chverificationType + '</button>';

                                containerer.appendChild(divElement);

                                // Add to table1
                                let newRowch = document.createElement('tr');
                                newRowch.innerHTML = `
                            <td>${chverificationType}</td>
                            <td>${totalAmount}</td>
                            <td>${taxAmount}</td>
                            <td class="total-amount">${grandAmount}</td>
                        `;
                                tbody1.appendChild(newRowch);

                                // Add to table2
                                let newRowch2 = document.createElement('tr');
                                newRowch2.innerHTML = `
                                <td>${chverificationType}</td>
                                <td><input type="text" name="chamount_${element}" class="input-amount" data-reserve-rate="${reserveRate}" onchange="updateLastTotal(this)" /></td>
                                <td>${totalAmount}</td>
                                  <td>${taxAmount}</td>
                                <td class="total-amount">${grandAmount}</td>
                            `;
                                tbody2.appendChild(newRowch2);
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#final_submit a[href$="#finish"]', function(e) {
                ...
                if (reminderSmsValue == '1') {
                    // Option 1: Set the reminder date to tomorrow
                    reminderSmsDate.setDate(reminderSmsDate.getDate() + 1);
                } else if (reminderSmsValue == '2') {
                    // Option 2: Set the reminder date to the day after tomorrow
                    reminderSmsDate.setDate(reminderSmsDate.getDate() + 2);
                } else if (reminderSmsValue == '3') {
                    // Option 3: Set the reminder date to 3 days after today
                    reminderSmsDate.setDate(reminderSmsDate.getDate() + 3);
                } else {
                    // Option 0: No reminder, leave as empty
                    reminderSmsDate = '';
                }
        ...
        function test() {
            ...
            if (reminderSms === '0' || reminderEmail === '0' || !activateDate || !validTill || !amount) {
