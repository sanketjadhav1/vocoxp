...
// Assuming these values are defined elsewhere in your code
    ...
    $sql_01 = "select * from construction_site_worker_header_all where agency_id='$agency_id' and site_id='$site_id' order by inserted_on ";
    ...
    if ($mysqli->affected_rows > 0) {
        while ($row_01 = mysqli_fetch_assoc($result_01)) {
         ...
         $responce = ["error_code" => 100, "message" => "Site workers data fetched","data" => $data];
        echo json_encode($responce);
        return;
    }else{
           $response = ["error_code" => 101, "message" => "No workers data found."];
            echo json_encode($response);
            exit;
    }
   ...
   $sql_02 = "update construction_site_worker_header_all set blocked_or_removed_on='$system_date_time',blocked_or_removed_reason='$reason',status='$status' where agency_id='$agency_id' and site_id='$site_id' and worker_id='$worker_id'"; 
        $result_02 = mysqli_query($mysqli, $sql_02);
        if ($mysqli->affected_rows > 0) {
            $responce[] = ["error_code" => 100, "message" => "updated Successfully"];
            echo json_encode($responce);
            return;
        }else{
