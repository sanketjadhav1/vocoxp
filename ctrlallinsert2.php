...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insert_upload_bulk = "INSERT INTO ` ` 
...
foreach ($data as $index => $row) {
    ...
    // Get all non-empty names, emails, and mobiles from the row
    $names = [];
    foreach ($nameIndices as $nameIndex) {
        if (!empty($row[$nameIndex])) {
            $names[] = trim($row[$nameIndex]);
        }
    }
    ...
    foreach ($mobileIndices as $mobileIndex) {
        if (!empty($row[$mobileIndex])) {
            $mobiles[] = trim($row[$mobileIndex]);
        }
    }

    // Now loop over the names, and for each name, try to find corresponding email and mobile
    foreach ($names as $name) {
        // Find corresponding email and mobile for each name (if exists)
        $email = $emails[0] ?? ''; // Get the first non-empty email
        $mobile = $mobiles[0] ?? ''; // Get the first non-empty mobile

        // Insert the record only if the name exists
        if (!empty($name)) {
            ...
            VALUES ('$agency_id', '$bulk_id', '$upload_id', '$end_user_id', '$type', '$name', '$mobile', '$email', '', '', '0', '$schedle', '$paid_by', '$reminder_email', '$reminder_sms')";
