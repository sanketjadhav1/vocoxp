    ...
    try {
        ...
        foreach ($data as $index => $row) {
            ...
            if ($type == 'Hostel Registration' || $excel_no == 1043) {
                ...
                $roles = ["student", "parent"]; // Ensure this has exactly 2 roles
                
                // Generate unique end_user_id values for both records
                $end_user_ids = [];
                // for ($i = 0; $i < 2; $i++) {
                //     $end_user_ids[$i] = unique_id_generate_bulk('END', 'bulk_end_user_transaction_all', $mysqli, "end_user_id");
                // }
                ...
                $email1 = $emails[0] ?? '';
                $role1 = $roles[0];
                $ref_enduser_id1 = $end_user_ids[1];

                // $values[] = "('$agency_id', '$bulk_id', '$upload_id', '{$end_user_ids[0]}', '$type', '$name1', '$mobile1', '$email1', '', '', '0', '$schedle', '$paid_by', '$reminder_email', '$reminder_sms', '$role1', '$ref_enduser_id1')";

                $name2 = $names[1] ?? '';
                $mobile2 = $mobiles[1] ?? '';
                $email2 = $emails[1] ?? '';
                $role2 = $roles[1];
                $ref_enduser_id2 = $end_user_ids[0];
                // $values[] = "('$agency_id', '$bulk_id', '$upload_id', '{$end_user_ids[1]}', '$type', '$name2', '$mobile2', '$email2', '', '', '0', '$schedle', '$paid_by', '$reminder_email', '$reminder_sms', '$role2', '$ref_enduser_id2')";
        ...
        if (!empty($values)) {
            ...
            if ($res_end_user) {
                $rows_inserted = true;
                echo json_encode(['status' => 'error', 'message' => "Records inserted"]);
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Failed to insert end user data: ' . mysqli_error($mysqli)]);
                exit();
            }
